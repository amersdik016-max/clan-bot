...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "الأوامر المتاحة:\n"
        "/myid - معرفة رقم الـ Telegram ID بتاعك\n"
...
async def myid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"الـ Telegram ID بتاعك هو: {user.id}\n"
        "لو عايز تبقى أدمن، ابعت الرقم ده لصاحب البوت يضيفه في ADMIN_IDS."
    )
...
async def setup_commands(app: Application):
    """تسجيل قائمة الأوامر العامة لكل الأعضاء + قائمة إضافية لكل أدمن بس."""
    from telegram import BotCommand, BotCommandScopeChat, BotCommandScopeDefault

    general_commands = [
        BotCommand("start", "بدء استخدام البوت"),
        BotCommand("myid", "معرفة رقم التليجرام بتاعك"),
        BotCommand("register", "تسجيل بياناتك في فري فاير"),
        BotCommand("submit_glory", "إرسال نقاط الجلوري للاعتماد"),
        BotCommand("mystats", "عرض إحصائياتك"),
        BotCommand("leaderboard", "ترتيب أعضاء الكلان"),
        BotCommand("donate", "تأكيد التبرع اليومي"),
        BotCommand("donate_status", "مين اتبرع ومين لأ"),
    ]
    admin_commands = general_commands + [
        BotCommand("pending", "عرض طلبات الجلوري المعلقة"),
        BotCommand("approve", "اعتماد طلب جلوري"),
        BotCommand("reject", "رفض طلب جلوري"),
        BotCommand("addpoints", "إضافة/خصم نقاط يدويًا"),
        BotCommand("broadcast", "إرسال رسالة لكل الأعضاء"),
    ]

    # القائمة العامة لكل الأعضاء
    await app.bot.set_my_commands(general_commands, scope=BotCommandScopeDefault())

    # قائمة موسّعة لكل أدمن، ظاهرة له بس في الشات الخاص بينه وبين البوت
    for admin_id in ADMIN_IDS:
        try:
            ...
            logger.warning("مقدرش أسجل أوامر الأدمن لـ %s: %s", admin_id, e)
